...
class UserTest < ActiveSupport::TestCase
  ...
  test 'name should not be too long' do
    @user.first_name = 'a' * 55
    @user.last_name = 'a' * 226
    assert_not @user.valid?
  end
  ...
  test 'email validation should accept valid adresses' do
    valid_addresses = %w[user@example.com USER@foo.COM THE_US-ER@foo.bar.org 
      first.last@foo.jp alice+bob@baz.cn karlene@zol.co.zw]
  ...
  test 'email validation should reject invalid adresses' do
    invalid_addresses = %w[user@example,com user_at_foo.org user.name@example.
      foo@bar_baz.jp foo@bar+baz.com]
    invalid_addresses.each do | invalid_address |
      @user.email = invalid_address
      assert_not @user.valid?, "#{invalid_address.inspect} should be invaild"
    end
  end
